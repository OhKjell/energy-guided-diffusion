...
parser.add_argument(
    "--epochs", default=4, type=int, help="number of epochs to train for"
)
...
parser.add_argument("--retina_index", default=4, type=int)
...
parser.add_argument("--gaussian_fit", default=0, type=int)
...
if __name__ == "__main__":
    ...
    with open(
        f"{basepath}/data/marmoset_data/responses/config_05.yaml", "rb"
    ) as config_file:
    ...
    #     continue
    if not fancy_nonlin and not learn_filter:
        epochs = 1
    if cells is None:
        cells = [
            x
    ...
    for cell in cells:
        ...
        if do_gaussian_fit:
            gaussian_fit_str = "_gf_1"
        else:
            gaussian_fit_str = ""
        model_dir = f"models/ln_models/ln_models_factorized_{filter_type}/{data_type}/retina{retina_index + 1}/cell_{cell}/lr_{lr}_whole_rf_{rf_size}_ch_{num_of_channels}_l1_{l1}_l2_{l2}_g_{gamma}_bs_{batch_size}_tr_{num_of_trials}_s_{subsample}_c_{crop}_n_{args.norm}_fn_{args.fancy_nonlin}_do_n_{args.do_nonlin}{gaussian_fit_str}/"
        if rf_size is not None or sta:
            ...
            if do_svd:
                ...
                if do_gaussian_fit:
                    _, _, fitted_sta = fit_gaussian(sta[0])
                    sta = fitted_sta, sta[1]
            size = sta[0].shape
        ...
        if wandb_log:
            wandb.init(
                config=config,
                project=f"ln-model-{data_type}-factorized-{filter_type}",
        ...
        for k, epoch in enumerate(range(epochs)):
            ...
            with torch.no_grad():
                ...
                print(model_dir)
                print(f"max cell {performance} corr:", max(epoch_correlations))
                ...
                if performance == "test":
                    ...
                    single_correlations = correlation(
                        all_predictions, all_responses, 1e-12
                    )
                    if device == "cuda":
                        single_correlations = single_correlations[0].item()
                # print('cell valid corr:', single_correlations)
                print(f"avg {performance} corr:", single_correlations)
                print(
                    f"avg {performance} loss:",
                    sum(epoch_valid_losses) / len(epoch_valid_losses),
                )
                print(f"max {performance} loss:", max(epoch_valid_losses))

                print(
                    f"avg {performance} variance:",
                    ...
                    wandb.log(
                        {
                            ...
                            f"loss/{perf_str}": sum(epoch_valid_losses)
                            / len(epoch_valid_losses),
                            f"variance/{perf_str}": sum(epoch_valid_variances)
            ...
            with torch.no_grad():
                ...
                if performance == "validation":
                    ...
                    np.save(f"{stats_dir}/correlations_test", np_correlations)
                ...
                mean_performance.append(np.max(correlations))
        if wandb_log:
            wandb.finish()
    print(f"Retina {retina_index+1} mean across cells:", np.mean(mean_performance))
